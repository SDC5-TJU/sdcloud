...
public class ContainerMonitorImpl implements ContainerMonitor {
	...
	// Map<String,TwoTuple<Float,Float>> containerNetUseMap=new
	// HashMap<String,TwoTuple<Float,Float>>();
	...
	// try {
	// sess=conn.openSession();
	// command="cat /proc/`docker inspect --format='{{ .State.Pid }}'
	// \""+containerName+"\"`/net/dev";
	// sess.execCommand(command);
	...
	// }
	//
	// containerNetUseMap.put(containerName,new
	// TwoTuple<Float,Float>(netIO[0],netIO[1]));
	// } catch (IOException e) {
	// e.printStackTrace();
	// }finally{
	// if(sess!=null)
	...
	public ArrayList<TableContainerresourceusage> getContainersPOJO(String hostname, String username, String password,
			InputStream input) {
		// Map<String,TwoTuple<Float,Float>>
		// containerNetUseMap=this.getContainerNetUsage();
		String command = "sudo docker stats --no-stream --format \"{{.Name}}:{{.CPUPerc}}:{{.MemUsage}}:{{.MemPerc}}:{{.NetIO}}:{{.BlockIO}}\"";
		Session sess = null;
		try {
			...
			while ((line = bfReader.readLine()) != null) {
				...
				if ("K".equalsIgnoreCase(mem.substring(mem.length() - 3, mem.length() - 2))) {
					...
					record.setMemusageamount(
							DataFormats.getInstance().subFloat(Float.parseFloat(mem) / 1024f / 1024f, 2));
				}
				...
				// 容器net 资源监控 start
				/*
				 * String[] netArray = split[4].split("/"); if
				 * (netArray[0].trim().endsWith("MB")) { record.setNetinput(
				 * Float.parseFloat(netArray[0].trim().substring(0,
				 * netArray[0].trim().length() - 2))); } else if
				 * (netArray[0].trim().endsWith("kB")) { record.setNetinput(
				 * Float.parseFloat(netArray[0].trim().substring(0,
				 * netArray[0].trim().length() - 2)) / 1024f); } else if
				 * (netArray[0].trim().endsWith("GB")) { record.setNetinput(
				 * Float.parseFloat(netArray[0].trim().substring(0,
				 * netArray[0].trim().length() - 2)) * 1024f); } else if
				 * (netArray[0].trim().endsWith("B") &&
				 * !(netArray[0].trim().endsWith("GB") ||
				 * netArray[0].trim().endsWith("kB") ||
				 * netArray[0].trim().endsWith("MB"))) { record.setNetinput(
				 * Float.parseFloat(netArray[0].trim().substring(0,
				 * netArray[0].trim().length() - 1)) / 1024f / 1024f); } if
				 * (netArray[1].trim().endsWith("MB")) { record.setNetoutput(
				 * Float.parseFloat(netArray[1].trim().substring(0,
				 * netArray[1].trim().length() - 2))); } else if
				 * (netArray[1].trim().endsWith("kB")) { record.setNetoutput(
				 * Float.parseFloat(netArray[1].trim().substring(0,
				 * netArray[1].trim().length() - 2)) / 1024f); } else if
				 * (netArray[1].trim().endsWith("GB")) { record.setNetoutput(
				 * Float.parseFloat(netArray[1].trim().substring(0,
				 * netArray[1].trim().length() - 2)) * 1024f); } else if
				 * (netArray[1].trim().endsWith("B") &&
				 * !(netArray[1].trim().endsWith("GB") ||
				 * netArray[1].trim().endsWith("kB") ||
				 * netArray[1].trim().endsWith("MB"))) { record.setNetoutput(
				 * Float.parseFloat(netArray[1].trim().substring(0,
				 * netArray[1].trim().length() - 1)) / 1024f / 1024f); }
				 */
				// record.setNetinput(Float.parseFloat(netArray[0].trim().substring(0,
				// netArray[0].trim().length() - 2)) * 1024f);
				// record.setNetoutput(Float.parseFloat(netArray[1].trim().substring(0,
				// netArray[1].trim().length() - 1))/ 1024f / 1024f);
